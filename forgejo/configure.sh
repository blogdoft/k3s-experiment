#!/usr/bin/env bash

set -e

. ../.env

PG_IMAGE="postgres:14-alpine"

PG_HOST="${DATABASE_HOST:-127.0.0.1}"
PG_PORT="${PG_PORT:-5432}"
PG_USER="${K3S_DB_USER:-postgres}"
PG_PASSWORD="${K3S_DB_PASSWORD:-postgres}"
PG_MAINTENANCE_DB="${PG_MAINTENANCE_DB:-postgres}"

DB_NAME="${FORGEJO_DB_NAME:-forgejodb}"
DB_USER="${FORGEJO_DB_USER:-forgejo}"
DB_PASSWORD="${FORGEJO_DB_PASSWORD:-passw0rd}"

run_psql() {
  docker run --rm \
    -e PGPASSWORD="${PG_PASSWORD}" \
    "${PG_IMAGE}" \
    psql \
      -h "${PG_HOST}" \
      -p "${PG_PORT}" \
      -U "${PG_USER}" \
      -d "${PG_MAINTENANCE_DB}" \
      -v ON_ERROR_STOP=1 \
      "$@"
}

echo "Verifying database: ${DB_NAME}"

DB_EXISTS=$(run_psql -tAc "SELECT 1 FROM pg_database WHERE datname = '${DB_NAME}';")

if [ "${DB_EXISTS}" = "1" ]; then
  echo "Database existe. Encerrando conexões e removendo: ${DB_NAME}"

  run_psql -c "
    SELECT pg_terminate_backend(pid)
    FROM pg_stat_activity
    WHERE datname = '${DB_NAME}'
      AND pid <> pg_backend_pid();
  "

  run_psql -c "DROP DATABASE \"${DB_NAME}\";"
else
  echo "Database não existe, nada para remover: ${DB_NAME}"
fi

echo "Verifying user: ${DB_USER}"

ROLE_EXISTS=$(run_psql -tAc "SELECT 1 FROM pg_roles WHERE rolname = '${DB_USER}';")

if [ "${ROLE_EXISTS}" = "1" ]; then
  echo "User already exists: ${DB_USER}"
  echo "Update user password: ${DB_USER}"

  run_psql -c "ALTER ROLE \"${DB_USER}\" WITH LOGIN PASSWORD '${DB_PASSWORD}';"
else
  echo "Create user: ${DB_USER}"

  run_psql -c "CREATE ROLE \"${DB_USER}\" WITH LOGIN PASSWORD '${DB_PASSWORD}';"

  run_psql -c "GRANT ALL PRIVILEGES ON DATABASE forgejodb TO forgejo";
fi

echo "Create database: ${DB_NAME}"

run_psql -c "
  CREATE DATABASE \"${DB_NAME}\"
  WITH
    OWNER \"${DB_USER}\"
    TEMPLATE template0
    ENCODING 'UTF8'
    LC_COLLATE 'en_US.UTF-8'
    LC_CTYPE 'en_US.UTF-8';
"

echo "Database recriado com sucesso."
echo
echo "Processing Configmap"
TEMP_FILE=configMap-temp.yaml
SOURCE_FILE=configmap.yaml
envsubst < "$SOURCE_FILE" > "$TEMP_FILE"
echo "Creating Configmap"
kubectl create namespace forgejo \
    --save-config \
    --dry-run=client -o yaml | kubectl apply -f -
kubectl apply -f $TEMP_FILE -n forgejo
rm $TEMP_FILE